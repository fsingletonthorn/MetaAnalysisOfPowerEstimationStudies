...
# replace file path with the file path to PowerEstimationReviewDataCollection2018.03.05.xlsx
...
forest(dat.back$yi, ci.lb=dat.back$ci.lb, ci.ub=dat.back$ci.ub, psize=1,
       xlim=c(-0.5,1.8), alim=c(0,1), ylim=c(-1,23), refline=NA, digits=3L, xlab="Proportion", slab = dat.back$PaperCitation)
addpoly(pred$pred, ci.lb=pred$ci.lb, ci.ub=pred$ci.ub, row=-0.5, digits=3, mlab="REML Model", efac=1.3)
abline(h=0.49)
text(-0.5, 22, "Study",               pos=4)
text( 1.8, 22, "Proportion [95% CI]", pos=2)
